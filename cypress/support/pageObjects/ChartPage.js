class ChartPage {
  elements = {
    welcomeContainer: () =>
      cy.get("#welcome.welcome-container", { timeout: 40000 }),
    createTemplatesBtn: () =>
      cy.get('[name="Create Templates"]', { timeout: 40000 }),

    layoutSection: () => cy.get("#section1.layout-style", { timeout: 40000 }),
    deleteButton: () => cy.get('[title="Delete Item"]', { timeout: 40000 }),
    ...
    propertiesTab: () =>
      cy.get('[data-testid="Properties"]', { timeout: 40000 }),
    alignmentGrid: () => cy.get(".grid-align-container", { timeout: 40000 }),
    centerCenterButton: () =>
      cy
        .get(".grid-align-container .btn")
        .contains("C | C", { timeout: 40000 }),
    ...
    chartTitle: () =>
      cy.get('[data-testid="Custom Chart Title"]', { timeout: 40000 }),

    resizeValueInput: () =>
      cy
        .get("div.size-container.ng-star-inserted > app-value-unit", {
          timeout: 40000,
        })
        .eq(3)
    ...
    errorMessage: () =>
      cy.get(".error-message, .validation-error", { timeout: 40000 }),
  ...
  createMasterPage() {
    ...
    cy.get('#welcome.welcome-container, [name="Create Templates"]', {
      timeout: 60000,
    })
      .should("exist")
      .then(($elements) => {
        if ($elements.length > 0) {
          ...
          if ($elements.is("#welcome.welcome-container")) {
            cy.get("#welcome.welcome-container", { timeout: 60000 }).should(
              "be.visible"
            );
          }
  ...
  deleteChart() {
    // First click on the chart to select it (this makes the delete button appear)
    this.elements.placedChart().click();
    
    // Wait for delete button to appear and then click it
    this.elements.deleteButton().should('be.visible', { timeout: 10000 }).click();
    
    // Verify the chart is deleted
    this.elements.placedChart().should('not.exist', { timeout: 10000 });
    
    return this;
  }

  checkIfChartExistsInSection() {
    // Check if chart exists in the layout section
    return cy.get('body').then(($body) => {
      const chartExists = $body.find('[data-testid="Chart1"]').length > 0;
      return chartExists;
    });
  }

  analyzeAndDeleteChartIfPresent() {
    cy.wait(4000);
    this.waitForPageLoad();
    // Analyze if chart is present in section, click on it and delete if found
    return this.checkIfChartExistsInSection().then((chartExists) => {
      if (chartExists) {
        cy.log('Chart found in section, clicking on it and deleting...');
        this.deleteChart();
        return true;
      } else {
        cy.log('No chart found in section, returning false');
        return false;
      }
    });
  }
  ...
  checkPageState() {
    ...
    cy.get("body").then(($body) => {
      ...
      const hasCreateTemplates =
        $body.find('[name="Create Templates"]').length > 0;
